...
def get_post_site_count(sql_context, start_date, end_date):
  ...
  """
  # queried GST, returned EST? would explain why it starts and ends four hours earlier in terms of numbers
...
def word_frequencies_for_month(sql_context, date:datetime, stop_words:set):
  ...
  df = reduce_word_counts(df, sum_post_word_counts_by_site(get_post_site_text(sql_context,
                          offset_time_string(month_start), offset_time_string(month_end))))
...
def get_post_site_size(sql_context, start_date, end_date):
  ...
  :return: a spark_session data frame, starts and ends four hours early relative to queried datetime range
  """
  query = "SELECT siteId, dateCreated, LENGTH(body) as size " + \
          "FROM " + SCHEMA + ".post " + \
          "WHERE dateCreated BETWEEN \"" + \
          start_date + "\" AND \"" + end_date + "\""
  return execute_query(sql_context, query)
...
def index_all_hourly_total_post_sizes(sql_context, start_date, end_date):
  ...
  print("index_all_hourly_total_post_sizes")
  start_date = start_date.replace(day=1)
  for n in range((end_date.year - start_date.year) * 12 + end_date.month - start_date.month):
    month = start_date + relativedelta(months=+n)
    print(month)
    query = "SELECT siteId, postId, body, dateCreated " + \
            "FROM " + SCHEMA + ".post " + \
            "WHERE dateCreated BETWEEN \"" + \
            offset_time_string(month) + "\" AND \"" + offset_time_string(month + relativedelta(months=+1)) + "\""
    df = execute_query(sql_context, query).withColumn("size", functions.length("body"))\
        .groupBy(functions.date_trunc('hour', 'dateCreated').alias("time"), 'siteId')\
        .agg(functions.sum('size').alias('size'))
    insert_df_to_table(df, "hourly_post_sizes")
    df.unpersist()
  return
